...
Data-driven methods for robotics have demonstrated an impressive ability to train robots to perform tasks from data collected autonomously or provided to the robot as task demonstrations. These methods hold a lot of promise for enabling robots to generalize and perform tasks in a variety of situations by learning from the right kind of experiences. 

The videos below are examples of recent data-driven methods that enabled robots to be robust to adversarial conditions, such as [human intervention](https://sites.google.com/view/data-driven-robotics/) or a [stuffed giraffe](https://openai.com/blog/solving-rubiks-cube/). However, when these methods are successful, they tend to converge to either one or a handful of task solutions, which prevents robots from generalizing to more varied scenarios that might require completely new strategies. 

<!-- TODO: add robust videos -->

{% figure %}
<video autoplay loop muted playsinline class="postimagehalf">
  <source src="{{ site.baseurl }}/assets/img/posts/2019-10-07-roboturk/robust.mp4" type="video/mp4">
</video>
<video autoplay loop muted playsinline class="postimagehalf">
  <source src="{{ site.baseurl }}/assets/img/posts/2019-10-07-roboturk/robust_1.mp4" type="video/mp4">
</video>

<figcaption>
Data-driven robotics is promising as a way to enable robots to generalize to many different situations.
</figcaption>
{% endfigure %}

<!-- Data-driven methods such as Reinforcement Learning and Self-Supervised Learning have demonstrated an impressive ability to collect large amounts of data autonomously and enable robotic skill learning from it. However, when these methods are successful, they tend to converge to either one or a handful of related task solutions, which prevents robots from generalizing to more varied scenarios. -->

Consider the example below where the robot has learned to grasp the bowl. The robot has learned one solution that enables it to grasp the bowl when it is upright (left).
However, it is unable to grasp the bowl when it is flipped over (middle). By contrast, **humans are able to easily generalize** to this new situation by realizing that they can simply push the bowl to the edge of the table, flip it, and then grasp it. This capacity to easily identify another solution in a novel task instance is enabled by diverse prior experience. 
...
For that reason, it is important to make sure robots can learn from the right kind of data. In this blog post, we describe how we built a data collection platform that enables collecting datasets that can help robots generalize. 
